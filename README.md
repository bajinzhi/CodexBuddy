...
Mobile support is currently in progress for CodexBuddy-owned iOS and Android clients.

- Current status: mobile layout runs, remote backend flow is wired, iOS/Android default to remote backend mode, and Android has Tauri config/scripts ready for initialization.
...
- Android note: `src-tauri/gen/android` is generated by `npm run tauri:android:init` on a machine with Android SDK/NDK installed.

### iOS/Android + Tailscale Setup (TCP)

Use this when connecting the mobile app to a desktop-hosted daemon over your Tailscale tailnet.
Canonical runbook: `docs/mobile-ios-tailscale-blueprint.md`.

1. Install and sign in to Tailscale on both desktop and the mobile device (same tailnet).
...
6. On mobile CodexBuddy, open `Settings > Server`.
...
Use the standalone daemon control CLI when you want mobile remote mode without keeping the desktop app open.
...
- Android SDK Platform, Platform-Tools, NDK (Side by side), Build-Tools, and Command-line Tools installed through Android Studio SDK Manager.
- `JAVA_HOME` points to the Android Studio JBR.
- `ANDROID_HOME` and `NDK_HOME` are set.
...
Initialize the generated Android project after prerequisites are installed:
...
Run on a connected emulator or USB device:
...
For physical-device dev, make sure the Vite dev server is reachable from the device. `tauri android dev --host` can be passed through npm when needed:
