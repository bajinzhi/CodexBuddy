...
pub(crate) fn daemon_binary_candidates() -> &'static [&'static str] {
    if cfg!(windows) {
        &["codex_buddy_daemon.exe", "codex-buddy-daemon.exe"]
    } else {
        &["codex_buddy_daemon", "codex-buddy-daemon"]
    }
}
...
pub(crate) fn resolve_daemon_binary_path() -> Result<PathBuf, String> {
    ...
    if let Ok(explicit_raw) = std::env::var("CODEX_BUDDY_DAEMON_PATH") {
...
mod tests {
    ...
    fn daemon_binary_candidates_prioritize_underscored_name() {
        assert!(daemon_binary_candidates()[0].starts_with("codex_buddy_daemon"));
    }
}
