...
export const PullRequestSummary = memo(function PullRequestSummary({
  ...
  const prUpdatedLabel = pullRequest.updatedAt
    ...
    : null;
  const prAuthor = pullRequest.author?.login ?? t("common:labels.unknown");
  ...
  return (
    <section className="diff-viewer-pr" aria-label={t("gitDiff.pullRequestSummary")}>
      <div className="diff-viewer-pr-header">
          ...
          <div className="diff-viewer-pr-header-actions">
            {hasDiffs && (
              <button
                ...
                aria-label={t("gitDiff.jumpToFirstFile")}
              >
            ...
            {onCheckoutPullRequest ? (
              <button
                ...
                aria-label={t("gitDiff.checkoutBranchAria", { number: pullRequest.number })}
              ...
              >
                {isCheckingOut ? t("gitDiff.checkingOut") : t("gitDiff.checkoutBranch")}
              </button>
            ) : null}
          </div>
        </div>
        <div className="diff-viewer-pr-meta">
          ...
          {pullRequest.isDraft && (
            <span className="diff-viewer-pr-pill">{t("gitDiff.draft")}</span>
          )}
        </div>
      </div>
      <div className="diff-viewer-pr-body">
        {prBody ? (
          ...
          <div className="diff-viewer-pr-empty">{t("gitDiff.noDescriptionProvided")}</div>
        )}
      </div>
      <div className="diff-viewer-pr-timeline">
        <div className="diff-viewer-pr-timeline-header">
          <span className="diff-viewer-pr-timeline-title">{t("gitDiff.activity")}</span>
          <span className="diff-viewer-pr-timeline-count">
            {t("gitDiff.commentCount", { count: sortedComments.length })}
          </span>
        ...
        <div className="diff-viewer-pr-timeline-list">
          ...
          {!pullRequestCommentsLoading &&
            ...
            !sortedComments.length && (
              <div className="diff-viewer-pr-timeline-state">
                {t("gitDiff.noCommentsYet")}
              </div>
            )}
          {hiddenCommentCount > 0 && !isTimelineExpanded && (
            <div className="diff-viewer-pr-timeline-divider">
              {t("gitDiff.earlierComment", { count: hiddenCommentCount })}
            </div>
          )}
          {visibleComments.map((comment) => {
            const commentAuthor = comment.author?.login ?? t("common:labels.unknown");
