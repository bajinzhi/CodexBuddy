...
export function SettingsAgentsSection({
  ...
  const handleCreateAgent = async () => {
    ...
    if (!name) {
      setCreateError(t("agents.errors.agentNameRequired"));
      return;
    }
    setCreateError(null);
  ...
  const handleUpdateAgent = async () => {
    ...
    if (!nextName) {
      setEditError(t("agents.errors.agentNameRequired"));
      return;
    }
  ...
  return (
    ...
    >
      <div className="settings-help settings-agents-builtins-help">
        {t("agents.builtinsHelp.before")} <code>default</code>, <code>explorer</code>,{" "}
        <code>worker</code> {t("agents.builtinsHelp.after")}
      </div>

      <SettingsToggleRow
        title={t("agents.configFileTitle")}
        subtitle={t("agents.configFileSubtitle", { fileManager: fileManagerName() })}
      >
      ...
      <SettingsToggleRow
        title={t("agents.enableTitle")}
        subtitle={
          t("agents.enableSubtitle")
        }
      >
        <SettingsToggleSwitch
      ...
      <SettingsToggleRow
        title={t("agents.maxThreadsTitle")}
        subtitle={
          t("agents.maxThreadsSubtitle", { min: MIN_MAX_THREADS, max: MAX_MAX_THREADS })
        }
      >
        <div className="settings-agents-stepper" role="group" aria-label={t("agents.maxThreadsAria")}>
          <button
            type="button"
            ...
            aria-label={t("agents.decreaseMaxThreads")}
          >
            ▼
          </button>
          ...
          <button
            ...
            aria-label={t("agents.increaseMaxThreads")}
          >
            ▲
          </button>
        </div>
      ...
      <SettingsToggleRow
        title={t("agents.maxDepthTitle")}
        subtitle={
          t("agents.maxDepthSubtitle", { min: MIN_MAX_DEPTH, max: MAX_MAX_DEPTH })
        }
      >
        <div className="settings-agents-stepper" role="group" aria-label={t("agents.maxDepthAria")}>
          <button
            type="button"
            className="ghost settings-agents-stepper-button"
            onClick={() => {
            ...
            aria-label={t("agents.decreaseMaxDepth")}
          >
            ▼
          </button>
          <div className="settings-agents-stepper-value" aria-live="polite" aria-atomic="true">
          ...
          <button
            ...
            aria-label={t("agents.increaseMaxDepth")}
          >
            ▲
          </button>
        </div>
      ...
      <div className="settings-field settings-agents-form">
        <div className="settings-agents-description-row">
          ...
          <button
            ...
            title={t("agents.generateFieldsTitle")}
            aria-label={t("agents.generateNewAgentAria")}
          >
            {createDescriptionGenerating ? (
        ...
        <label className="settings-label" htmlFor="settings-agent-create-developer-instructions">
          {t("agents.developerInstructionsLabel")}
        </label>
        <textarea
          ...
          placeholder={t("agents.developerInstructionsPlaceholder")}
          disabled={creatingAgent}
        />
        <div className="settings-agents-model-row">
          <div className="settings-agents-model-field settings-agents-model-field--model">
            <span className="settings-agents-inline-label">{t("agents.modelInlineLabel")}</span>
            <select
          ...
          <div className="settings-agents-model-field settings-agents-model-field--effort">
            <span className="settings-agents-inline-label">{t("agents.reasoningInlineLabel")}</span>
            <select
              ...
              aria-label={t("agents.agentReasoningAria")}
            >
              {createReasoningOptions.length === 0 && <option value="">{t("agents.notSupported")}</option>}
        ...
        <div className="settings-agents-actions">
          <button type="button" className="ghost" onClick={() => void handleCreateAgent()}>
            {creatingAgent ? t("common:status.creating") : t("agents.createButton")}
          </button>
        </div>
        {modelOptions.length === 0 && (
          <div className="settings-help">
            {modelOptionsLoading
              ? t("agents.modelFallbackLoading")
              : t("agents.modelFallbackReady")}
          </div>
        )}
      ...
      <SettingsSubsection
        title={t("agents.configuredTitle")}
        subtitle={t("agents.configuredSubtitle")}
      />

      {settings && settings.agents.length === 0 && !isLoading && (
        <div className="settings-help">{t("agents.noneConfigured")}</div>
      )}
        ...
        return (
          <div className="settings-field settings-agent-card" key={agent.name}>
            ...
            <div className="settings-help settings-help-inline">
              <code>{agent.configFile || t("agents.missingConfigFile")}</code>
            </div>
            <div className="settings-agents-actions">
              {!isPendingDelete && (
                <>
              ...
              >
                {isReadingConfig ? t("common:status.opening") : t("agents.editFile")}
              </button>
              {!agent.managedByApp && (
                <span className="settings-help settings-help-inline">{t("agents.externalPath")}</span>
              )}
            </div>
            {isPendingDelete && (
              <div className="settings-agents-actions">
                <span className="settings-help settings-help-inline">
                  {t("agents.deletePrompt")}
                </span>
                <button
                  type="button"
                  className="ghost"
                ...
                <button
                  ...
                  disabled={isDeleting}
                >
                  {isDeleting ? t("common:status.deleting") : t("agents.confirmDelete")}
                </button>
              </div>
            )}
              ...
              <div className="settings-agents-edit-form">
                <div className="settings-agents-description-row">
                  ...
                  <button
                    ...
                    title={t("agents.generateFieldsTitle")}
                    aria-label={t("agents.generateExistingAgentAria", { name: agent.name })}
                  >
                    {editDescriptionGenerating ? (
                      <MagicSparkleLoaderIcon className="settings-agents-generate-loader" />
                    ) : (
                ...
                <div className="settings-agents-actions">
                  ...
                  <button
                    ...
                    disabled={isUpdating}
                  >
                    {isUpdating ? t("common:status.saving") : t("common:actions.save")}
                  </button>
                </div>
                ...
                <div className="settings-agents-header">
                  <div>
                    <div className="settings-toggle-title">
                      {t("agents.configFileTitleForAgent", { name: agent.name })}
                    </div>
                    <div className="settings-toggle-subtitle">
                  ...
                  <div className="settings-agents-actions">
                    ...
                    >
                      {isWritingConfig ? t("common:status.saving") : t("common:actions.save")}
                    </button>
                  </div>
                </div>
                <textarea
      ...
      {isLoading && <div className="settings-help">{t("agents.loading")}</div>}
