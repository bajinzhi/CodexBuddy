...
export async function saveModelProviderSettings(
  ...
  return invoke<ModelProviderSettings>("save_model_provider_settings", {
    input,
  });
}
...
export async function sendUserMessage(
  ...
  const images = options?.attachments
    ? null
    : await normalizeImagesForRpc(options?.images);
...
export async function updateThreadSettings(
  workspaceId: string,
  threadId: string,
  settings: ThreadSettingsUpdateInput,
) {
  return invoke<any>("thread_settings_update", {
    workspaceId,
    threadId,
    settings,
  });
